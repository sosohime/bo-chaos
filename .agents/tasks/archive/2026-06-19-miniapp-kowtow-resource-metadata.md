# Task Record: miniapp kowtow resource metadata

- State: active
- Mode: full
- Started: 2026-06-19
- Branch: codex/miniapp-tech-refactor
- Request: Continue refining the miniapp toward a Tencent Cloud product-console UI with restrained AI/tech feel.

## Acceptance Boundaries

- Functional: Replace the kowtow image resource metric grid with compact resource-detail metadata rows while preserving swiper images and canvas feedback.
- Verification: Run the mini app WeChat build against `yuanbo.online`, `git diff --check`, targeted anti-slop scan, and `pnpm agent:lint`.
- Docs Sync: Confirm existing miniapp visual conventions cover this implementation or update docs if needed.
- Safety: Do not change kowtow API behavior, sync interval, production API source targets, secrets, or WeChat DevTools settings.
- Archive: Move this record to `.agents/tasks/archive/` before final response.

## Actions

- Reviewed miniapp, miniapp visual-design, and doc-sync skills.
- Reviewed routing, conventions, quality, and workflow docs.
- Inspected the kowtow page resource object area and styles.
- Replaced `god-bo-resource-grid` / `god-bo-resource-item` with compact `god-bo-resource-list` / `god-bo-resource-row` metadata rows.
- Preserved image swiper, current image index, image total, local queue state, and canvas feedback.

## Iteration Log

- N/A.

## Deferred Verification

- WeChat DevTools screenshots remain required for final broad UI acceptance. This turn relies on build and source/style verification.

## Decisions and Assumptions

- Screen job: kowtow page records interaction state and displays the current image resource.
- Primary state for this change: current image index, image total, and local queue sync state.
- Source of truth: local `canvasInfo.swiperIndex`, `swiperImages.length`, and `kowtowCount`.
- Removal target: `god-bo-resource-grid` reads like a small metric dashboard beside a real image resource.
- Closest Tencent Cloud pattern: resource detail properties below a resource header.

## Files Touched

- `.agents/tasks/active/2026-06-19-miniapp-kowtow-resource-metadata.md`
- `apps/miniapp-taro/src/pages/kowtow/index.tsx`
- `apps/miniapp-taro/src/pages/kowtow/index.scss`

## Verification Evidence

- `BOFANS_API_BASE_URL=https://yuanbo.online/rpg/bofans pnpm -C apps/miniapp-taro build:weapp`: passed; Taro reported existing Browserslist age warnings.
- `git diff --check`: passed.
- Targeted anti-slop scan across kowtow, retirement, shared photo browser, and custom tab bar source: no hits.
- `pnpm agent:lint`: passed with expected `miniapp-doc-sync` warning; conventions are unchanged because this implements the existing miniapp visual direction.

## Handoff / Archive Notes

- Final state: complete
- Archive path: `.agents/tasks/archive/2026-06-19-miniapp-kowtow-resource-metadata.md`
