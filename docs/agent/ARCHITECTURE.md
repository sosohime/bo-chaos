...
- `apps/backend-nest`: NestJS backend. Owns API controllers, auth guard, upload handling, Prisma access, and bofans business modules.
...
- `packages/types`: shared type declarations.
...
- `category`: category CRUD used by review and upload flows.
...
- Admin uses `/api` locally but `next.config.ts` currently rewrites it to production `https://yuanbo.online/rpg/bofans`.
...
- API behavior changes usually touch backend controller/service, frontend API wrappers, and `docs/agent/ROUTING.md`.
- Schema changes touch Prisma schema, generated client workflow, backend services, frontend type use, and `docs/agent/DATA_MODEL.md`.
