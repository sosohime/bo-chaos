...
- `sitePath()` removes the deploy prefix only during Astro development so all local routes/assets work; `siteUrl()` always includes the production prefix, independent of dev mode. Production preview must mount `dist/` at `/retire` and its homepage at `/`.
...
- Bo news and Bo retirement use the newer `src/components/editorial/RetirementClock.astro`; the previous component still serves other countdown pages. Keep copy immediate and honestly labelled; never simulate AI generation delays. Use shared start/target calendar dates in UTC+8, clamp progress to 0–100%, and stop timer work while the page is hidden.
- Editorial pages share `SiteShell.astro` and semantic CSS tokens. Theme defaults to system; explicit light/dark choice persists in `bo-appearance` with storage-failure tolerance. Surfaces use 12px radii, controls 6px, and all changed controls need keyboard/reduced-motion checks.
- Curated source records in `src/data/news.ts` have permanent IDs used by details, sitemap and RSS. Never confuse `publishedAt` with `observedAt`, fabricate publication dates, or attribute product-wide updates to Yuanbo personally. Product banners remain outbound records; curated record pages use WebPage plus a cited source, not invented NewsArticle authorship.
- News filtering is progressive enhancement: keep all records and source links in static HTML. `q` and `category` URL parameters restore on reload/history traversal. Publication dates, canonical identity, existing anchors, and the fan-maintained disclaimer survive visual changes.
